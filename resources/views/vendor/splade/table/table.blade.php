<SpladeTable {{ $attributes->except('class') }} :striped="@js($striped)" :columns="@js($table->columns())" :search-debounce="@js($searchDebounce)" :default-visible-toggleable-columns="@js($table->defaultVisibleToggleableColumns())" :items-on-this-page="@js($table->totalOnThisPage())" :items-on-all-pages="@js($table->totalOnAllPages())" :base-url="@js(request()->url())">
    <template #default="{!! $scope !!}">
        <div {{ $attributes->only('class')->merge([
            'class' => 'relative overflow-x-auto',
        ]) }} :class="{ 'opacity-50': table.isLoading }">
            @if ($hasControls())
            ...
            <x-splade-component is="table-wrapper">
                <table class="min-w-full divide-y divide-base-100 bg-red">
                    @unless ($headless)
                        @isset($head)
                            {{ $head }}
