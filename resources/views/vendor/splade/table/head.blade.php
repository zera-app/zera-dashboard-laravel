<thead class="bg-base-300">
    <tr>
        ...
        <th v-show="table.columnIsVisible(@js($column->key))"
            class="@if($loop->first && $hasBulkActions) pr-6 @else px-6 @endif py-3 text-left text-xs font-medium tracking-wide text-base-content {{ $column->classes }}">
            @if($column->sortable)
            ...
            <span
                ...
                <svg aria-hidden="true"
                    class="w-3 h-3 ml-2 @if($column->sorted) text-green-500 @else text-gray-400 @endif"
