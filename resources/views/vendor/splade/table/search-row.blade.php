<div v-show="@js($searchInput->value !== null) || table.isForcedVisible(@js($searchInput->key))"
    class="px-4 sm:px-0 mb-3">
    <div class="flex rounded-md shadow-sm relative mt-3">
        <label for="{{ $searchInput->key }}"
            class="inline-flex items-center px-4 rounded-l-md border border-r-0 border-base-300 bg-base-300 text-base-content text-sm">
        ...
        <input name="searchInput-{{ $searchInput->key }}" value="{{ $searchInput->value }}" type="text"
            class="bg-base-100 text-base-content flex-1 min-w-0 block w-full px-3 py-2 rounded-none rounded-r-md rounded-l-sm border focus:ring-secondary-focus focus:border-secondary-focus text-sm border-base-300"
